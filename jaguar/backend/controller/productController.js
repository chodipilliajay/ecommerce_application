...
// 2️⃣Get all products
// Top discounted products, for the "Best Deals" home page carousel.
// Uses an aggregation to compute discount % (cutPrice vs price) and sort by it.
export const getDealProducts = handleAsyncError(async (req, res, next) => {
  ...
  const deals = await Product.aggregate([
    { $match: { cutPrice: { $gt: 0 } } },
    {
      $addFields: {
        discountPercent: {
          $round: [
            { $multiply: [{ $divide: [{ $subtract: ["$cutPrice", "$price"] }, "$cutPrice"] }, 100] },
            0,
          ],
        },
      },
...
// Most recently added products, for a "New Arrivals" home page carousel.
